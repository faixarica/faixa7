...
def get_db():
    try:
        conn = sqlite3.connect('database.db')  # Conecta ao banco de dados (ou cria se não existir)
        return conn
...
def inicializar_banco_dados():
    ...
    # Verifica e cria a tabela usuarios, se necessário
    ...
    # Verifica e cria a tabela planos, se necessário
    ...
    # Verifica e cria a tabela client_plans, se necessário
    if not tabela_existe(cursor, "client_plans"):
        cursor.execute("""
            CREATE TABLE client_plans (
                ...
                data_expira_plan TEXT NOT NULL DEFAULT (datetime('now', '+30 days')), -- Valor padrão: 30 dias após inclusão
    ...
    # Verifica e cria a tabela resultados_oficiais, se necessário
    ...
    # Verifica e cria a tabela palpites, se necessário
...
# Chamada adicional para inicializar o banco de dados quando o script for executado diretamente
